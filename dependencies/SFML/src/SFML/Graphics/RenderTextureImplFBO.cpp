...
bool RenderTextureImplFBO::create(unsigned int width, unsigned int height, unsigned int textureId, bool depthBuffer)
{
	...
	if (depthBuffer)
	{
		...
		glCheck(GLEXT_glRenderbufferStorage(GLEXT_GL_RENDERBUFFER, GL_DEPTH24_STENCIL8_EXT, width, height));
		...
		glCheck(GLEXT_glFramebufferRenderbuffer(GLEXT_GL_FRAMEBUFFER, GL_STENCIL_ATTACHMENT_EXT, GLEXT_GL_RENDERBUFFER, m_depthBuffer));
